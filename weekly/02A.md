...
- [HW - Ajax-2 - loading and parsing CSV files](https://github.com/tonethar/IGME-330-Master/blob/master/notes/HW-ajax-2.md)
  ...
  - The above is a legitimate use for the CSV format, but more typically, CSV is used to store multiple *records*, where each line of the CSV file is a distinct *entity*. For example each line of a **customers.csv** file could represent information about the customer, and the various pieces of data about that customer are separated by commas (`lastName`, `firstName`, `address`, `city`, `zip`, `countryCode` and so on)
