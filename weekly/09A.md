...
- Other stuff to keep in mind about performance testing:
  - Order matters because of browser memory management, so swapping the order of these tests will give you different results. Automated performance tools often handle this for you
  - In your applications, if you have long running activities that might lock up the browser UI, consider a [Web Worker](https://developer.mozilla.org/en-US/docs/Web/API/Web_Workers_API/Using_web_workers)
